...
class dangocoClient extends tunnelClient{
	constructor(options,settings){
		...
		options.addr=dangocoClient.generateAddr(options,settings);
	...
	proxy(protocol,addr,port,clientStream,callback){
		...
		clientStream.once('end',()=>{
			clientStream._waitProxy&&this.removeListener('tunnel_open',clientStream._waitProxy);
		...
		if(!this.tunnelCreated){
			clientStream._waitProxy=()=>{
				clientStream._waitProxy=null;
				this.proxy(protocol,addr,port,clientStream,callback);
			}
			this.once('tunnel_open',clientStream._waitProxy);
			return;
		}
