...
class dangocoClient extends tunnelClient{
	constructor(options,settings){
		...
		dangocoClient.generateAddr(options,settings);
		
		super(options);//tunnelClient constroctor
		this.settings=Object.assign({},settings);
		this.proxyList=new Set();

		this.connectionMng.once('idle',()=>{
			this.close();
		});
	}
	proxy(protocol,addr,port,stream){
		stream&&stream.pause();//pause the stream,otherwise the data will escape
	...
	_TCPProxy(addr,port,clientStream){
		...
		let piped=false;
		clientStream&&tunnelStream.once('tunnel_stream_open',()=>{
			...
			if(!piped)
				clientStream.destroy(`Tunnel stream error: ${e.message}`);
		});
		return tunnelStream;
	}
...
*/
	...
	static generateAddr(options,settings){
		...
		query.set(urlSafeBase64.encode(aes256cfb('s',settings.pass)),md5(aes256cfb(settings.pass,settings.pass)));
		if(settings.algo){
