...
class dangocoServer extends events{
	constructor(opt,callback){
		...
		this.tunnelServer.on('tunnel_open',tunnel=>{
			if(tunnel.keepBrokenTunnel>opt.maxBrokenTunnelTimeout*1000)tunnel.keepBrokenTunnel=opt.maxBrokenTunnelTimeout*1000;
	...
	parseWsRequest(info,cb){
		...
		let argCount=0;
		for(let a of querys.keys()){
			argCount++;
		}
		if(argCount>10){//reject request with too many query parameters
			cb(false,500,'');
			return;
		}
		this.getPass(user,(pass)=>{
			...
			//auth user
			try{
				if(!params.s)throw('wrong password');
				let clientTimeBuf=de_aes256cfb(urlSafeBase64.decode(params.s),pass),
					clientTime=new Date(clientTimeBuf.readUIntBE(0,clientTimeBuf.byteLength));
				if(Date.now()-clientTime <= 5*60*1000){//5 minutes
					req._user=user;
					req._pass=pass;
				}else{
					throw('auth timeout');
