...
class dangocoServer extends events{
	constructor(opt,callback){
		...
		this.blockRules.set(Symbol('blocked-address'),(type,addr)=>{//rule:blocked address
			if(this.BLOCKEDADDR.has(addr))return true;
		});
		this.blockRules.set(Symbol('private-address'),(type,addr)=>{//rule:private address
			if(type<=4){//ip
	...
	isBlockedAddr(addr){
		...
		for(let [name,rule] of this.blockRules){
			if(rule(type,addr)===true)return true;
		}
	}
}
...
class ProxyStream extends events{
	constructor(server,stream,algo,user,key){
		...
		this.inStream.once('data',this._headBuilder);
	}
	...
	_udpModeStream(){
		...
		this.dangocoUDP.on('frame',(frame,headLength)=>{
			...
			if(this.server.isBlockedAddr(addr)){
				return;
			}
